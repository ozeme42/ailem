...
export default function DashboardScreen() {
  ...
  const activeGoals = useMemo(() =>
    goals.filter(g => g.status === 'in-progress').map(goal => {
      ...
      const totalUnits = goal.totalUnits || 0;
      const overallProgress = totalUnits > 0 ? (done / totalUnits) * 100 : 0;

      const secDone = currentSection?.completedUnits || 0;
      const secTotal = currentSection?.sectionTotalUnits || 0;
      const sectionProgress = secTotal > 0 ? (secDone / secTotal) * 100 : 0;
      ...
      <View style={{ paddingHorizontal: isTabletLandscape ? 0 : 14, paddingTop: 14 }}>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', gap: 6 }}>
            {[
              { label: 'Görev', value: pendingTasksCount, emoji: '📋', route: '/tasks' },
              { label: 'Ödev', value: pendingTestsCount + pendingStudyAssignmentsCount, emoji: '🎯', route: '/education' },
              { label: 'Ödüller', value: '⭐', emoji: '🌟', route: '/rewards' },
              { label: 'Alışveriş', value: shoppingCount, emoji: '🛒', route: '/shopping' },
              { label: 'Kitap', value: books.length, emoji: '📚', route: '/library' },
            ].map(s => (
              <TouchableOpacity key={s.label} onPress={() => router.push(s.route as any)} style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.13)', borderRadius: 14, paddingVertical: 10, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.16)' }}>
                <Text style={{ fontSize: 14, marginBottom: 2 }}>{s.emoji}</Text>
                <Text style={{ color: 'white', fontWeight: '900', fontSize: 15, lineHeight: 20 }}>{s.value}</Text>
                <Text style={{ color: 'rgba(255,255,255,0.75)', fontWeight: '700', fontSize: 9, marginTop: 1 }}>{s.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </LinearGradient>
  ...
  const renderBentoGrid = () => {
    return (
      <View style={{ paddingHorizontal: isTabletLandscape ? 0 : 14, gap: 10 }}>
        ...
        {/* Davranış & Ödüller — Tam genişlikte kart */}
        <TouchableOpacity onPress={() => router.push('/rewards' as any)} activeOpacity={0.85}>
          <LinearGradient
            colors={['#A259FF', '#FF6B9D', '#FF9500']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={{ borderRadius: 22, padding: 18, flexDirection: 'row', alignItems: 'center', overflow: 'hidden' }}
          >
            <View style={{ position: 'absolute', right: -20, top: -20, opacity: 0.15 }}>
              <Gift size={100} color="white" />
            </View>
            <View style={{ width: 52, height: 52, borderRadius: 26, backgroundColor: 'rgba(255,255,255,0.25)', alignItems: 'center', justifyContent: 'center', marginRight: 14 }}>
              <Text style={{ fontSize: 26 }}>⭐</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ color: 'white', fontWeight: '900', fontSize: 16, letterSpacing: -0.3 }}>Davranış & Ödüller</Text>
              <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 11, fontWeight: '600', marginTop: 3 }}>Çocuklar için yıldız sistemi — Ödül Marketi</Text>
            </View>
            <View style={{ backgroundColor: 'rgba(255,255,255,0.25)', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6 }}>
              <Text style={{ color: 'white', fontWeight: '800', fontSize: 12 }}>Aç →</Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
      </View>
        ...
        <View style={{ backgroundColor: cardBg, borderRadius: 22, padding: 16, borderWidth: 1, borderColor: cardBorder, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 }}>
          ...
          <View style={{ gap: 12 }}>
            {activeGoals.slice(0, 3).map((goal, idx) => {
              ...
              const unitLabel = goal.unitName || goal.unit || 'birim';
              return (
                    ...
                    <View style={{ paddingLeft: 16, paddingRight: 14, paddingVertical: 12 }}>
                      ...
                      <View style={{ marginBottom: 8 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                          <Text style={{ color: textSub, fontSize: 11, fontWeight: '700' }}>
                            Genel İlerleme: <Text style={{ color: c1, fontWeight: '900' }}>{goal.done} / {goal.totalUnits || 0}</Text> {unitLabel}
                          </Text>
                          <Text style={{ color: c1, fontWeight: '900', fontSize: 11 }}>%{Math.round(goal.overallProgress || 0)}</Text>
                        </View>
                        <View style={{ height: 6, borderRadius: 3, backgroundColor: isDark ? 'rgba(255,255,255,0.09)' : 'rgba(0,0,0,0.07)', overflow: 'hidden' }}>
                          <LinearGradient colors={[c1, c2]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ height: '100%', borderRadius: 3, width: `${Math.min(100, goal.overallProgress || 0)}%` as any }} />
                        </View>
                      </View>

                      {/* 2. AKTİF BÖLÜM / BİRİM İLERLEMESİ */}
                      {goal.currentSection && (
                        <View style={{ paddingTop: 6, borderTopWidth: 1, borderTopColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)' }}>
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                            <Text style={{ color: textMain, fontSize: 10, fontWeight: '700', flex: 1, marginRight: 6 }} numberOfLines={1}>
                              Bölüm: <Text style={{ fontWeight: '800' }}>{goal.currentSection.title}</Text>
                            </Text>
                            <Text style={{ color: c2, fontWeight: '900', fontSize: 10 }}>
                              {goal.currentSection.completedUnits || 0} / {goal.currentSection.sectionTotalUnits} {unitLabel} (%{Math.round(goal.sectionProgress || 0)})
                            </Text>
                          </View>
                          <View style={{ height: 5, borderRadius: 2.5, backgroundColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)', overflow: 'hidden' }}>
                            <LinearGradient colors={[c2, c1]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ height: '100%', borderRadius: 2.5, width: `${Math.min(100, goal.sectionProgress || 0)}%` as any }} />
                          </View>
                        </View>
                      )}
