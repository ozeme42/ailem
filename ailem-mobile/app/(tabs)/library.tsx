import { View, Text, ScrollView, TouchableOpacity, ActivityIndicator, Image, TextInput, Modal, StyleSheet, Dimensions, Animated, KeyboardAvoidingView, Platform, Alert, RefreshControl, useWindowDimensions, NativeModules } from 'react-native';
...
export default function LibraryScreen() {
  ...
  // Sync book reading data to Android home screen widget
  useEffect(() => {
    if (Platform.OS !== 'android' || !NativeModules.WidgetModule) return;
    if (!familyMembers || allBooks.length === 0 || userLibraries.length === 0) return;
    try {
      const widgetData = familyMembers.map(member => {
        const lib = userLibraries.find(l => l.memberId === member.id);
        const readingBooks = (lib?.books || [])
          .filter(b => b.status === 'reading')
          .slice(0, 3)
          .map(b => {
            const bookInfo = allBooks.find(bk => bk.id === b.bookId);
            const totalPages = bookInfo?.pageCount || 0;
            // progress is stored as 0-100 percentage
            const progress = b.progress || 0;
            const currentPage = totalPages > 0 ? Math.round((progress / 100) * totalPages) : 0;
            return {
              title: bookInfo?.title || 'Kitap',
        ...
        return { memberName: member.name, books: readingBooks };
      }).filter(m => m.books.length > 0);
      NativeModules.WidgetModule.setBookData(JSON.stringify(widgetData));
    } catch (err) { console.log('Book widget sync error', err); }
  }, [allBooks, userLibraries, familyMembers]);
  ...
  // Helper: Record remaining unread pages as a reading session when finishing a book
  const recordFinishBookSession = async (bookId: string) => {
    if (!selectedMember) return;
    const bookDetail = allBooks.find(b => b.id === bookId);
    if (!bookDetail) return;

    const totalPages = bookDetail.pageCount || 0;
    const sessionsForBook = readingSessions.filter(
      s => s.memberId === selectedMember.id && s.bookId === bookId
    );
    const pagesRecordedInSessions = sessionsForBook.reduce(
      (sum, s) => sum + (s.pagesRead || 0), 0
    );

    let unreadPages = 0;
    if (totalPages > 0) {
      unreadPages = Math.max(0, totalPages - pagesRecordedInSessions);
    }
    ...
    if (unreadPages > 0) {
      try {
        await addReadingSession({
          ...
          durationSeconds: 0,
          pagesRead: unreadPages,
        });
      } catch (e) {
        console.log('Error adding reading session for remaining pages:', e);
      }
    }
  };
  ...
  const handleUpdateStatus = async (bookId: string, newStatus: 'reading' | 'finished', progress?: number) => {
    ...
    if (newStatus === 'finished') {
      ...
      await recordFinishBookSession(bookId);
    }

    try {
      await updateUserBookStatus(familyId, selectedMember.id, bookId, newStatus, progress ?? 100);
  ...
  const handleUploadAndFinish = async () => {
    ...
    try {
      ...
      // Helper: Android content:// URIs need XMLHttpRequest, not fetch()
      const uriToBlob = (uri: string): Promise<Blob> =>
        new Promise((resolve, reject) => {
          const xhr = new XMLHttpRequest();
          xhr.onload = () => resolve(xhr.response);
          xhr.onerror = () => reject(new Error('URI okuma hatası: ' + uri));
      ...
      // Record remaining unread pages in reading sessions for stats!
      await recordFinishBookSession(childBookToFinish.id);
