...
import { ChevronLeft, Search, Plus, Filter, X, Star, BookOpen, Edit, Check, ImagePlus, Play, Trash2 } from 'lucide-react-native';
...
export default function LibraryAllScreen() {
  ...
  const displayTags = useMemo(() => {
    return Array.from(new Set([...DEFAULT_LIBRARY_SHELVES, ...allTags]));
  }, [allTags]);

  const booksByShelf = useMemo(() => {
     const grouped = new Map<string, Book[]>();
     
     displayTags.forEach(tag => grouped.set(tag, []));
  ...
  const handleDeleteBook = (book: Book) => {
    Alert.alert(
      "Kitabı Sil",
      `"${book.title}" kitabını kütüphaneden ve tüm üyelerin okuma listelerinden tamamen silmek istediğinize emin misiniz?`,
      [
        { text: "İptal", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert("Başarılı", "Kitap kütüphaneden silindi.");
            } catch (e) {
                          ...
                          <View className="p-6 pt-5 flex-col gap-4">
                              ...
                              <View className="flex-row gap-3 mt-6">
                                  ...
                                  <TouchableOpacity 
                                    ...
                                    className="w-14 h-14 rounded-2xl border border-red-200 dark:border-red-900/30 items-center justify-center bg-red-50 dark:bg-red-950/30 active:opacity-75"
                                  >
      ...
      <Modal visible={isEditModalOpen} transparent={true} animationType="slide">
          <View className="flex-1 bg-black/50 justify-end">
              ...
              <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                <View 
                   className="bg-white dark:bg-[#1C1C1E] p-6 rounded-t-[32px] border-t border-black/5 dark:border-white/5"
                   style={{ height: '80%' }}
                >
                    ...
                    <View className="flex-row gap-3">
                        <TouchableOpacity 
                            onPress={() => selectedBook && handleDeleteBook(selectedBook)}
                            className="w-14 h-12 bg-red-100 dark:bg-red-950/40 rounded-xl items-center justify-center border border-red-200 dark:border-red-900/30 active:opacity-85"
                        >
                            <Trash2 size={20} color="#EF4444" />
                        </TouchableOpacity>
                        <TouchableOpacity 
                            onPress={handleSaveEdit}
                            className="flex-1 h-12 bg-[#16a085] rounded-xl items-center justify-center active:opacity-85 shadow-sm shadow-[#16a085]/25"
                        >
