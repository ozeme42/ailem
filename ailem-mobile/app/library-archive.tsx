...
import { ChevronLeft, Search, Plus, X, Trash2, Edit, Check, Settings, BookOpen, Layers, Star, ImagePlus, LayoutGrid, LayoutList, Columns } from 'lucide-react-native';
...
export default function LibraryArchiveScreen() {
  ...
  const [layoutMode, setLayoutMode] = useState<'shelves' | 'grid' | 'list' | 'bookshelf'>('shelves');
        ...
        <View className="flex-1">
            ...
            <View className="px-4 py-3 gap-3" style={{
              ...
              borderBottomWidth: 1
                ...
                {/* Görünüm Seçenekleri Switcher Bar */}
                <View className="flex-row items-center justify-between px-1 pt-1">
                    <Text className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Görünüm Seçenekleri</Text>
                    <View className="flex-row items-center p-1 rounded-xl" style={{
                      backgroundColor: isDark ? 'rgba(0, 0, 0, 0.4)' : 'rgba(226, 232, 240, 0.5)',
                    }}>
                        <TouchableOpacity 
                          onPress={() => setLayoutMode('shelves')}
                          className="px-2.5 py-1 rounded-lg flex-row items-center gap-1"
                          style={layoutMode === 'shelves' ? { backgroundColor: '#16a085' } : {}}
                        >
                          <Layers size={13} color={layoutMode === 'shelves' ? 'white' : (isDark ? '#94a3b8' : '#64748b')} />
                          <Text className={`text-[10px] font-bold ${layoutMode === 'shelves' ? 'text-white' : 'text-slate-500 dark:text-slate-400'}`}>Raflar</Text>
                        </TouchableOpacity>

                        <TouchableOpacity 
                          onPress={() => setLayoutMode('grid')}
                          className="px-2.5 py-1 rounded-lg flex-row items-center gap-1"
                          style={layoutMode === 'grid' ? { backgroundColor: '#16a085' } : {}}
                        >
                          <LayoutGrid size={13} color={layoutMode === 'grid' ? 'white' : (isDark ? '#94a3b8' : '#64748b')} />
                          <Text className={`text-[10px] font-bold ${layoutMode === 'grid' ? 'text-white' : 'text-slate-500 dark:text-slate-400'}`}>Izgara</Text>
                        </TouchableOpacity>

                        <TouchableOpacity 
                          onPress={() => setLayoutMode('list')}
                          className="px-2.5 py-1 rounded-lg flex-row items-center gap-1"
                          style={layoutMode === 'list' ? { backgroundColor: '#16a085' } : {}}
                        >
                          <LayoutList size={13} color={layoutMode === 'list' ? 'white' : (isDark ? '#94a3b8' : '#64748b')} />
                          <Text className={`text-[10px] font-bold ${layoutMode === 'list' ? 'text-white' : 'text-slate-500 dark:text-slate-400'}`}>Liste</Text>
                        </TouchableOpacity>

                        <TouchableOpacity 
                          onPress={() => setLayoutMode('bookshelf')}
                          className="px-2.5 py-1 rounded-lg flex-row items-center gap-1"
                          style={layoutMode === 'bookshelf' ? { backgroundColor: '#16a085' } : {}}
                        >
                          <Columns size={13} color={layoutMode === 'bookshelf' ? 'white' : (isDark ? '#94a3b8' : '#64748b')} />
                          <Text className={`text-[10px] font-bold ${layoutMode === 'bookshelf' ? 'text-white' : 'text-slate-500 dark:text-slate-400'}`}>Kitaplık</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </View>
            ...
            <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
                {filteredBooks.length === 0 ? (
                ...
                ) : layoutMode === 'grid' || searchQuery.length > 0 ? (
                    /* IZGARA GÖRÜNÜMÜ (3 Column Grid) */
                    ...
                    /* DETAYLI LİSTE GÖRÜNÜMÜ (1 Column Detailed Cards) */
                    <View className="gap-3">
                        {filteredBooks.map(book => (
                            ...
                            >
                                <View className="w-14 h-20 rounded-xl bg-slate-200 dark:bg-slate-800 overflow-hidden mr-3 border border-black/5 dark:border-white/10">
                                  <Image source={{ uri: book.image || 'https://placehold.co/300x450.png' }} className="w-full h-full object-cover" />
                                </View>
                                <View className="flex-1 justify-center">
                                  <Text className="text-sm font-bold text-slate-800 dark:text-slate-100 mb-0.5" numberOfLines={1}>
                                    {book.title}
                                  </Text>
                                  <Text className="text-xs font-semibold text-[#16a085] mb-2" numberOfLines={1}>
                                    {book.author || 'Bilinmeyen Yazar'}
                                  </Text>
                                  <View className="flex-row items-center gap-3">
                                    ...
                                    {book.tags && book.tags.length > 0 && (
                                      <View className="bg-emerald-500/10 px-2 py-0.5 rounded-md">
                                        <Text className="text-[10px] font-bold text-[#16a085]">{book.tags[0]}</Text>
                                      </View>
                                    )}
                                  </View>
                                </View>
                    ...
                    /* AHŞAP KİTAPLIK GÖRÜNÜMÜ (Wooden Bookshelf Rows) */
                           ...
                           return (
                               <View key={shelfName}>
                                   <View className="flex-row items-center justify-between mb-2 px-1">
                                       <Text className="text-base font-bold text-amber-900 dark:text-amber-200">🪵 {shelfName}</Text>
                                       <View className="bg-amber-900/10 dark:bg-amber-100/10 px-2.5 py-0.5 rounded-full">
                                         <Text className="text-[10px] font-bold text-amber-800 dark:text-amber-300">{booksInShelf.length} Kitap</Text>
                                       </View>
                                   </View>

                                   <ScrollView horizontal showsHorizontalScrollIndicator={false} className="-mx-4 px-4 pt-1">
                                       {booksInShelf.map((book, index) => (
                                           <TouchableOpacity 
                                             ...
                                             className="w-[100px] mr-4 items-center"
                                           >
                                               <View className="w-full aspect-[2/3] bg-slate-200 dark:bg-slate-800 rounded-lg overflow-hidden shadow-lg border border-black/10 dark:border-white/20 mb-1">
                                                 <Image source={{ uri: book.image || 'https://placehold.co/300x450.png' }} className="w-full h-full object-cover" />
                                               </View>
                                               <Text className="text-[11px] font-bold text-slate-800 dark:text-slate-100 text-center leading-tight" numberOfLines={1}>
                                                 {book.title}
                                               </Text>
                                           </TouchableOpacity>
                                       ))}
                                   </ScrollView>
                                   {/* Realistic Wooden Shelf Bar */}
                                   <LinearGradient 
                                     colors={['#78350F', '#451A03', '#290E02']} 
                                     start={{ x: 0, y: 0 }} 
                                     end={{ x: 1, y: 0 }} 
                                     className="h-3.5 w-full rounded-full shadow-lg border-t border-amber-400/40 mt-1" 
                                   />
                               </View>
                           );
                       })}
                    ...
                    /* RAFLAR GÖRÜNÜMÜ (Default Shelves Rows) */
                    <View className="gap-8">
                       {Array.from(booksByShelf.entries()).map(([shelfName, booksInShelf]) => {
                           if (booksInShelf.length === 0) return null;
