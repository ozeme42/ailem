...
const DEFAULT_CURRICULUM: Record<string, string[]> = {
  "Matematik": ["Temel Kavramlar", "Sayılar", "Bölünebilme", "EBOB-EKOK", "Rasyonel Sayılar", "Denklemler", "Üslü Sayılar", "Köklü Sayılar", "Problemler", "Mantık", "Kümeler", "Fonksiyonlar", "Polinomlar", "Trigonometri", "Logaritma", "Diziler", "Limit", "Türev", "İntegral"],
  "Türkçe": ["Sözcükte Anlam", "Cümlede Anlam", "Paragrafta Anlam", "Ses Bilgisi", "Yazım Kuralları", "Noktalama İşaretleri", "Dil Bilgisi", "Anlatım Bozukluğu"],
  "Fizik": ["Fizik Bilimine Giriş", "Madde ve Özellikleri", "Hareket ve Kuvvet", "İş, Güç ve Enerji", "Isı ve Sıcaklık", "Elektrik ve Magnetizma", "Basınç ve Kaldırma Kuvveti", "Dalgalar", "Optik"],
  "Kimya": ["Kimya Bilimi", "Atom ve Periyodik Sistem", "Kimyasal Türler Arası Etkileşimler", "Maddenin Halleri", "Mol Kavramı", "Tepkime Türleri", "Karışımlar", "Asitler, Bazlar ve Tuzlar", "Organik Kimya"],
  "Biyoloji": ["Yaşam Bilimi Biyoloji", "Hücre", "Canlıların Çeşitliliği", "Hücre Bölünmeleri", "Kalıtım", "Ekoloji", "İnsan Fizyolojisi", "Bitki Biyolojisi"],
  "Tarih": ["Tarih ve Zaman", "İlk Çağ Uygarlıkları", "Türk Dünyası", "İslam Tarihi", "Selçuklu Tarihi", "Osmanlı Tarihi", "İnkılap Tarihi"],
  "Coğrafya": ["Doğa ve İnsan", "Dünya'nın Şekli ve Hareketleri", "Harita Bilgisi", "İklim Bilgisi", "Nüfus ve Yerleşme", "Türkiye Coğrafyası"],
  "Felsefe": ["Felsefeyi Tanıma", "Bilgi Felsefesi", "Varlık Felsefesi", "Ahlak Felsefesi", "Sanat Felsefesi", "Din Felsefesi"],
  "Din Kültürü": ["İnanç", "İbadet", "Ahlak ve Değerler", "Hz. Muhammed (S.A.V.)"],
  "İngilizce": ["Kelime Bilgisi", "Gramer", "Okuma - Anlama", "Yazma"],
  "Genel / Edebiyat": ["Roman", "Hikaye", "Şiir", "Kişisel Gelişim", "Tarih & Araştırma", "Deneme & Test"]
};
...
export default function NewTrackedBookScreen() {
  ...
  const activeCurriculum = useMemo(() => {
    return { ...DEFAULT_CURRICULUM, ...(curriculumMap || {}) };
  }, [curriculumMap]);
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        {mode === 'single' ? (
          <View className="space-y-5">
            ...
            <View>
              <Text className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider mb-2">Ders / Raf Seçimi</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
                {Object.keys(activeCurriculum).map((subj) => {
              ...
              <View style={{ marginTop: 8 }}>
                <TextInput
                  placeholder="Veya özel ders / raf adı yazın..."
                  placeholderTextColor="#94a3b8"
                  value={subject}
                  onChangeText={(val) => {
                    setSubject(val);
                  ...
                  className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl px-4 py-2.5 text-slate-800 dark:text-white font-semibold text-xs"
                  style={{ borderRadius: 14 }}
                />
              </View>
            </View>
            ...
            {Boolean(subject) && (
              <View>
                <Text className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider mb-2">Konu Seçimi (Opsiyonel)</Text>
                {activeCurriculum[subject] && activeCurriculum[subject].length > 0 && (
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, marginBottom: 8 }}>
                    {activeCurriculum[subject].map((t) => {
                ...
                <TextInput
                  placeholder="Veya özel konu / bölüm adı yazın..."
                  placeholderTextColor="#94a3b8"
                  value={topic}
