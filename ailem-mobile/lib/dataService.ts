...
export const deleteBook = async (id: string) => {
    try {
        const libsSnap = await getDocs(collection(db, 'userLibraries'));
        for (const libDoc of libsSnap.docs) {
            const libData = libDoc.data() as UserLibrary;
            if (libData.books && libData.books.some(b => b.bookId === id)) {
                const updatedBooks = libData.books.filter(b => b.bookId !== id);
                await updateDoc(libDoc.ref, { books: updatedBooks }).catch(() => {});
            }
        }
    } catch (e) {
        console.error("Error cleaning up userLibraries on deleteBook:", e);
    }
    return deleteDoc(doc(db, "mediaItems", id));
};
...
// Accepts both File (web) and Blob (React Native mobile)
export const uploadImageToStorage = async (file: Blob | File, path: string): Promise<string> => {
...
export const uploadFileToStorage = async (file: Blob | File, path: string): Promise<string> => {
    const storageRef = ref(storage, path);
    const uploadTask = uploadBytesResumable(storageRef, file as Blob);
    await uploadTask;
    return await getDownloadURL(storageRef);
...
export const uploadFileUriToStorage = async (uri: string, path: string): Promise<string> => {
    // Use XMLHttpRequest to handle Android content:// URIs (fetch fails on Android)
    const blob: Blob = await new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.onload = () => resolve(xhr.response);
        xhr.onerror = () => reject(new Error('URI okuma hatası'));
...
export const addTrackedBook = async (data: any) => {
    const familyId = await getCurrentFamilyId();
    if (!familyId) throw new Error("Unauthorized");
    return addDoc(collection(db, 'trackedBooks'), { subjects: [], ...data, familyId, createdAt: new Date().toISOString() });
};
...
export const deleteTrackedBook = async (id: string) => {
    try {
        const testsSnap = await getDocs(query(collection(db, 'trackedBookTests'), where('bookId', '==', id)));
        for (const docItem of testsSnap.docs) {
            await deleteDoc(docItem.ref).catch(() => {});
        }
    } catch (e) {
        console.error("Error deleting book tests:", e);
    }
    return deleteDoc(doc(db, "trackedBooks", id));
};
...
export const deleteNotebook = async (id: string) => {
    try {
        const notesSnap = await getDocs(query(collection(db, 'notes'), where('notebookId', '==', id)));
        for (const docItem of notesSnap.docs) {
            await deleteDoc(docItem.ref).catch(() => {});
        }
    } catch (e) {
        console.error("Error deleting notebook notes:", e);
    }
    return deleteDoc(doc(db, 'notebooks', id));
};
...
export const onBehaviorRecordsUpdate = (familyId: string, callback: (records: BehaviorRecord[]) => void) => {
    const q = query(collection(db, 'behaviorRecords'), where('familyId', '==', familyId), orderBy('createdAt', 'desc'));
    return onSnapshot(q, (snap) => {
        callback(snap.docs.map(d => ({ id: d.id, ...d.data() } as BehaviorRecord)));
    });
};

export const addBehaviorRecord = async (familyId: string, memberId: string, data: {
    ...
    const record: any = {
        ...
        title: data.title,
        stars: data.stars,
        createdBy: data.createdBy || 'Ebeveyn',
        memberName: data.memberName,
        createdAt: new Date().toISOString(),
    };
    if (data.note) record.note = data.note;
    await addDoc(collection(db, 'behaviorRecords'), record);

    // Update the member's star balance
    const familyRef = doc(db, 'families', familyId);
    const familySnap = await getDoc(familyRef);
    if (familySnap.exists()) {
        const members = familySnap.data().members || [];
        ...
        if (idx > -1) {
            const currentBalance = members[idx].starBalance ?? 0;
            const currentTotal = members[idx].totalStarsEarned ?? 0;
            const delta = data.type === 'positive' ? data.stars : -data.stars;
            const newBalance = Math.max(0, currentBalance + delta);
            const newTotal = data.type === 'positive' ? currentTotal + data.stars : currentTotal;
            members[idx] = { ...members[idx], starBalance: newBalance, totalStarsEarned: newTotal };
            await updateDoc(familyRef, { members });
        }
    }
};

export const deleteBehaviorRecord = async (familyId: string, recordId: string) => {
    const recordRef = doc(db, 'behaviorRecords', recordId);
    const recordSnap = await getDoc(recordRef);
    if (!recordSnap.exists()) return;
    
    const recordData = recordSnap.data();
    const { memberId, type, stars } = recordData;
    ...
    if (familySnap.exists()) {
        ...
        if (idx > -1) {
            ...
            const newBalance = type === 'positive' 
                ? Math.max(0, currentBalance - stars)
                : currentBalance + stars;
            
            const newTotal = type === 'positive'
                ? Math.max(0, currentTotal - stars)
                : currentTotal;
...
// Reward Items (Market)
export const onRewardItemsUpdate = (familyId: string, callback: (items: RewardItem[]) => void) => {
    const q = query(collection(db, 'rewardItems'), where('familyId', '==', familyId));
    return onSnapshot(q, (snap) => {
        callback(snap.docs.map(d => ({ id: d.id, ...d.data() } as RewardItem)));
    });
};

export const addRewardItem = async (data: Omit<RewardItem, 'id'>) => {
    return addDoc(collection(db, 'rewardItems'), data);
};

export const updateRewardItem = (id: string, data: Partial<RewardItem>) => updateDoc(doc(db, 'rewardItems', id), data);
export const deleteRewardItem = (id: string) => deleteDoc(doc(db, 'rewardItems', id));

// Reward Claims
export const onRewardClaimsUpdate = (familyId: string, callback: (claims: RewardClaim[]) => void) => {
    const q = query(collection(db, 'rewardClaims'), where('familyId', '==', familyId), orderBy('claimedAt', 'desc'));
    return onSnapshot(q, (snap) => {
        callback(snap.docs.map(d => ({ id: d.id, ...d.data() } as RewardClaim)));
    });
};

export const claimReward = async (familyId: string, memberId: string, memberName: string, reward: RewardItem) => {
    return addDoc(collection(db, 'rewardClaims'), {
        familyId,
        memberId,
        memberName,
        rewardId: reward.id,
...
export const approveRewardClaim = async (familyId: string, claimId: string, memberId: string, starCost: number, resolvedBy: string) => {
    // Update claim status
    ...
    if (familySnap.exists()) {
        ...
        if (idx > -1) {
            const currentBalance = members[idx].starBalance ?? 0;
            members[idx] = { ...members[idx], starBalance: Math.max(0, currentBalance - starCost) };
            await updateDoc(familyRef, { members });
        }
    }
};

export const rejectRewardClaim = async (claimId: string, resolvedBy: string) => {
    return updateDoc(doc(db, 'rewardClaims', claimId), {
        status: 'rejected',
        resolvedAt: new Date().toISOString(),
        resolvedBy,
    });
...
// --- STICKER SYSTEM (2-tier) ---
export const getFamilyRewardSettings = async (familyId: string): Promise<FamilyRewardSettings> => {
    const ref = doc(db, 'rewardSettings', familyId);
    const snap = await getDoc(ref);
    if (snap.exists()) return { familyId, ...snap.data() } as FamilyRewardSettings;
    return { familyId, ...DEFAULT_REWARD_SETTINGS };
};

export const onRewardSettingsUpdate = (familyId: string, callback: (s: FamilyRewardSettings) => void) => {
    const ref = doc(db, 'rewardSettings', familyId);
    return onSnapshot(ref, (snap) => {
        if (snap.exists()) callback({ familyId, ...snap.data() } as FamilyRewardSettings);
        else callback({ familyId, ...DEFAULT_REWARD_SETTINGS });
    });
};

export const saveFamilyRewardSettings = async (familyId: string, settings: Partial<Omit<FamilyRewardSettings, 'familyId'>>) => {
    const ref = doc(db, 'rewardSettings', familyId);
    await setDoc(ref, { ...settings }, { merge: true });
};

export const onStickerRecordsUpdate = (familyId: string, callback: (records: StickerRecord[]) => void) => {
    const q = query(collection(db, 'stickerRecords'), where('familyId', '==', familyId), orderBy('createdAt', 'desc'));
    return onSnapshot(q, (snap) => {
        callback(snap.docs.map(d => ({ id: d.id, ...d.data() } as StickerRecord)));
    });
};

// Etiket ver: X yıldız harca (veya harcamadan), 1 etiket ekle
export const awardSticker = async (
    familyId: string,
    memberId: string,
    memberName: string,
    starsPerSticker: number = 10,
    count: number = 1,
    createdBy: string = 'Ebeveyn',
    note?: string,
    deductStars: boolean = true
) => {
    const validStarsPerSticker = Number(starsPerSticker) || 10;
    const validCount = Number(count) || 1;
    const starsSpent = deductStars ? validStarsPerSticker * validCount : 0;
    ...
    const record: any = {
        ...
        starsSpent,
        createdAt: new Date().toISOString(),
        createdBy: createdBy || 'Ebeveyn',
    };
    if (note) record.note = note;
    await addDoc(collection(db, 'stickerRecords'), record);
    ...
    if (familySnap.exists()) {
        ...
        const idx = members.findIndex((m: any) => m.id === memberId);
        if (idx > -1) {
            const currentStarBalance = Number(members[idx].starBalance) || 0;
            const currentStickerBalance = Number(members[idx].stickerBalance) || 0;
            const currentTotalStickers = Number(members[idx].totalStickersEarned) || 0;

            const starBalance = Math.max(0, currentStarBalance - starsSpent);
            const stickerBalance = currentStickerBalance + validCount;
            const totalStickersEarned = currentTotalStickers + validCount;

            members[idx] = { ...members[idx], starBalance, stickerBalance, totalStickersEarned };
            await updateDoc(familyRef, { members });
        }
    }
};

// Büyük ödül ver: X etiket harca, büyük ödül sayacını artır
export const awardBigReward = async (
    familyId: string,
    memberId: string,
    memberName: string,
    stickersPerBigReward: number = 10,
    createdBy: string = 'Ebeveyn',
    rewardTitle: string = 'Büyük Ödül',
    note?: string
) => {
    const validStickersNeeded = Number(stickersPerBigReward) || 10;
    ...
    const record: any = {
        ...
        count: validStickersNeeded,
        createdAt: new Date().toISOString(),
        createdBy: createdBy || 'Ebeveyn',
    };
    if (note || rewardTitle) record.note = note || rewardTitle;
    await addDoc(collection(db, 'stickerRecords'), record);
    ...
    if (familySnap.exists()) {
        ...
        if (idx > -1) {
            const currentStickerBalance = Number(members[idx].stickerBalance) || 0;
            const currentTotalBigRewards = Number(members[idx].totalBigRewardsEarned) || 0;

            const stickerBalance = Math.max(0, currentStickerBalance - validStickersNeeded);
            const totalBigRewardsEarned = currentTotalBigRewards + 1;

            members[idx] = { ...members[idx], stickerBalance, totalBigRewardsEarned };
            await updateDoc(familyRef, { members });
        }
    }
};

// Manuel etiket düzeltme (etiket geri al)
export const removeSticker = async (
    ...
    if (familySnap.exists()) {
        ...
        const idx = members.findIndex((m: any) => m.id === memberId);
        if (idx > -1) {
            const stickerBalance = Math.max(0, (members[idx].stickerBalance ?? 0) - count);
            members[idx] = { ...members[idx], stickerBalance };
            await updateDoc(familyRef, { members });
        }
    }
};

export const addOrUpdateBehaviorOption = async (familyId: string, option: BehaviorOption) => {
    const settings = await getFamilyRewardSettings(familyId);
    let behaviors = settings.customBehaviors;
    ...
    const idx = behaviors.findIndex(b => b.id === option.id);
    if (idx > -1) {
        behaviors[idx] = option;
    } else {
        behaviors.push(option);
    }
    await saveFamilyRewardSettings(familyId, { customBehaviors: behaviors });
};

export const deleteBehaviorOption = async (familyId: string, optionId: string) => {
    const settings = await getFamilyRewardSettings(familyId);
    let behaviors = settings.customBehaviors;
    if (!behaviors || behaviors.length === 0) {
        behaviors = [
    ...
    behaviors = behaviors.filter(b => b.id !== optionId);
    await saveFamilyRewardSettings(familyId, { customBehaviors: behaviors });
};

export const saveMemberRewardTarget = async (familyId: string, memberId: string, target: MemberRewardTarget) => {
    const settings = await getFamilyRewardSettings(familyId);
    const memberTargets = { ...(settings.memberTargets || {}), [memberId]: target };
    await saveFamilyRewardSettings(familyId, { memberTargets });
};
