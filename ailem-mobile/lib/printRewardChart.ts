...
try {
  ...
  console.warn('expo-print module missing:', e);
}

try {
  SharingModule = require('expo-sharing');
} catch (e) {
  console.warn('expo-sharing module missing:', e);
}
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .stars-single-row {
        display: grid;
        grid-template-columns: repeat(${starsPerSticker}, 1fr); /* STRICT SINGLE LINE NON-WRAPPING GRID */
        gap: 3px;
        width: 100%;
      }
      .mini-tick-box {
...
export const printOrShareRewardChart = async (
  member: FamilyMember,
  settings: FamilyRewardSettings,
  behaviors: BehaviorOption[]
) => {
  ...
  if (PrintModule) {
    ...
    if (SharingModule && await SharingModule.isAvailableAsync()) {
      await SharingModule.shareAsync(uri, {
        ...
        dialogTitle: `${member.name} - Etiket ve Yıldız Çizelgesi`,
      });
    } else {
      await PrintModule.printAsync({ html });
    }
  } else {
    console.warn('PrintModule is not available on this device.');
  }
};
