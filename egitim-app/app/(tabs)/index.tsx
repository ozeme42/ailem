...
// FORCE_RELOAD_TIMESTAMP=639189392908482496
// FORCE_RELOAD_TIMESTAMP=639189392448943332
// FORCE_RELOAD_TIMESTAMP=639189390848354582
// FORCE_RELOAD_TIMESTAMP=639189390127078060
// FORCE_RELOAD_TIMESTAMP=639189386573228889
// FORCE_RELOAD_TIMESTAMP=639189383757918857
// FORCE_RELOAD_TIMESTAMP=639189382037108413
import { 
  View, 
  Text, 
  ScrollView, 
...
export default function EducationScreen() {
  ...
  const handleSaveSingle = async () => {
    ...
    try {
      const newTest: Omit<Test, 'id' | 'profile?.id' | 'questions'> = {
  ...
  const renderFocusBar = () => {
    ...
    return (
      <Animated.View entering={FadeInDown.delay(50).springify()} style={{ paddingHorizontal: isTabletLandscape ? 0 : 20, marginBottom: isTabletLandscape ? 0 : 20 }}>
        <LinearGradient
          colors={isOverdue ? ['#7f1d1d', '#991b1b'] : ['#312e81', '#4c1d95']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
          style={{ borderRadius: 28, padding: 20, flexDirection: 'row', alignItems: 'center', gap: 16, shadowColor: '#6d28d9', shadowOffset: { width: 0, height: 16 }, shadowOpacity: 0.45, shadowRadius: 24, elevation: 10 }}
        >
          {/* Left icon */}
          ...
          >
            <Animated.View style={[
              ...
              {focusTask
                ? <Play size={20} color="#4f46e5" fill="#4f46e5" style={{ marginLeft: 3 }} />
  ...
  const renderStatsGrid = () => {
    ...
    const statCards: { label: string; value: string | number; icon: any; colors: GC; iconColor: string; ringValue?: number }[] = [
      { label: 'Tamamlama', value: '%' + Math.round(stats.completedAssignmentsRate), icon: CheckCircle2, colors: ['#064e3b', '#065f46'] as const, iconColor: '#6ee7b7' },
      { label: 'Başarı Oranı', value: '%' + stats.successRate.toFixed(1).replace('.', ','), icon: Award, colors: ['#312e81', '#4338ca'] as const, iconColor: '#c4b5fd', ringValue: stats.successRate },
      { label: 'Bekleyen', value: stats.pendingCount, icon: Clock, colors: ['#78350f', '#92400e'] as const, iconColor: '#fbbf24' },
      { label: 'Geciken', value: stats.overdueCount, icon: AlertCircle, colors: (stats.overdueCount > 0 ? ['#7f1d1d', '#991b1b'] : ['#0f172a', '#1e293b']) as GC, iconColor: stats.overdueCount > 0 ? '#fca5a5' : '#475569' },
    ];
    return (
      <View style={{ paddingHorizontal: isTabletLandscape ? 0 : 20, marginBottom: isTabletLandscape ? 0 : 24 }}>
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
          {statCards.map((card, idx) => {
            ...
            return (
              <Animated.View key={idx} entering={FadeInDown.delay(80 + idx * 70).springify()} style={{ width: '47%', flexGrow: 1 }}>
                <LinearGradient
                  colors={card.colors}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
                  style={{ borderRadius: 24, padding: 18, minHeight: 118, justifyContent: 'space-between', borderWidth: 1, borderColor: 'rgba(255,255,255,0.13)', shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.4, shadowRadius: 18, elevation: 10 }}
                >
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ width: 38, height: 38, borderRadius: 13, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' }}>
                      <Icon size={19} color={card.iconColor} />
                    </View>
                    {card.ringValue !== undefined && (
                      <Svg width={38} height={38} viewBox="0 0 36 36" style={{ transform: [{ rotate: '-90deg' }] }}>
                        <Circle cx="18" cy="18" r={13} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="3" />
                        <Circle cx="18" cy="18" r={13} fill="none" stroke="#c4b5fd" strokeWidth="3"
                          strokeDasharray={2 * Math.PI * 13}
                          strokeDashoffset={2 * Math.PI * 13 - (2 * Math.PI * 13 * (card.ringValue || 0)) / 100}
                          strokeLinecap="round"
                        />
                      </Svg>
                    )}
                  ...
                  <View>
                    <Text style={{ fontSize: 30, fontWeight: '900', color: 'white', letterSpacing: -1 }}>{card.value}</Text>
                    <Text style={{ fontSize: 10, fontWeight: '800', color: 'rgba(255,255,255,0.58)', marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.6 }}>{card.label}</Text>
                  </View>
                </LinearGradient>
              </Animated.View>
            );
  ...
  const renderQuickActions = () => {
    type GC = readonly [string, string, ...string[]];
    const actions: { name: string; desc: string; icon: any; colors: GC; route: string }[] = [
      { name: 'Konu Planı', desc: 'Ders Yol Haritası', icon: BookMarked, colors: ['#4c1d95', '#6d28d9'] as const, route: '/study-plans' },
      { name: 'Ders Özetleri', desc: 'Akıllı Not Kartları', icon: ScrollText, colors: ['#064e3b', '#065f46'] as const, route: '/summaries' },
      { name: 'Sınav Analiz', desc: 'Net Dağılımları', icon: ListTree, colors: ['#1e1b4b', '#3730a3'] as const, route: '/education-results' },
      { name: 'Hatalarım', desc: 'Tüm Yanlış Sorular', icon: AlertCircle, colors: ['#7f1d1d', '#991b1b'] as const, route: '/mistakes' },
    ];
    return (
              ...
              <Animated.View key={idx} entering={FadeInDown.delay(160 + idx * 60).springify()} style={{ width: isTabletLandscape ? '100%' : '47%', flexGrow: 1 }}>
                <TouchableOpacity onPress={() => router.push(item.route as any)} activeOpacity={0.75}>
                  <LinearGradient
                    ...
                    style={{ borderRadius: 22, padding: 18, flexDirection: 'row', alignItems: 'center', gap: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.13)', shadowColor: '#000', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.3, shadowRadius: 14 }}
                  >
                          ...
                          >
                            ...
                            <View style={{ flex: 1 }}>
                              ...
                              <Text style={{ fontSize: 8, fontWeight: '600', color: textSub, marginTop: 1 }}>
                                {a.subject} {a.dueDate ? `â€¢ ${format(safeParseDate(a.dueDate), 'dd MMM', { locale: tr })}` : ''}
                              </Text>
                            </View>
                          </TouchableOpacity>
                        ))}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg }}>
      ...
      <Animated.View entering={FadeInDown.springify()}>
        <LinearGradient
          colors={isDark ? ['#0f0c29', '#302b63', '#24243e'] : ['#4f46e5', '#7c3aed']}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            <View style={{ alignItems: 'center', flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7, marginBottom: 4 }}>
                <GraduationCap size={20} color="#fcd34d" />
                <Text style={{ color: 'white', fontSize: 20, fontWeight: '900', letterSpacing: -0.5 }}>Öğrenme Modülü</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                <Sparkles size={10} color="#fcd34d" />
                <Text style={{ color: 'rgba(255,255,255,0.72)', fontSize: 10, fontWeight: '800', letterSpacing: 1.4, textTransform: 'uppercase' }}>Kişisel Akademi</Text>
                <Sparkles size={10} color="#fcd34d" />
              </View>
            </View>
      ...
      {/* â”€â”€ PREMIUM OFFLINE RESULT MODAL â”€â”€ */}
      <Modal
