...
export default function BankQuestionsScreen() {
  ...
  const handleSaveQuestion = async () => {
    if (!formSubject.trim() || !formTopic.trim()) {
      Alert.alert('Hata', 'LÃ¼tfen ders ve konu giriniz.');
      return;
    }
    if (!formImageUri && !editingQuestion) {
      Alert.alert('Hata', 'LÃ¼tfen bir soru gÃ¶rseli seÃ§in.');
      return;
    }
    ...
    try {
      ...
      if (formImageUri && formImageUri !== editingQuestion?.imageUrl) {
        ...
        const path = `bankQuestions/${profile?.id}/${Date.now()}`;
      ...
      if (editingQuestion) {
        ...
        Alert.alert('BaÅŸarÄ±lÄ±', 'Soru gÃ¼ncellendi.');
      } else {
        await addBankQuestion(questionData);
        Alert.alert('BaÅŸarÄ±lÄ±', 'Soru bankasÄ±na eklendi.');
      }
  ...
  const handleDeleteSelected = () => {
    const performDelete = async () => {
      ...
      try {
        ...
        Alert.alert('BaÅŸarÄ±lÄ±', 'SeÃ§ilenler silindi.');
        setSelectedIds([]);
      } catch (e) {
        Alert.alert('Hata', 'Silme iÅŸlemi gerÃ§ekleÅŸtirilemedi.');
      } finally {
        setLoading(false);
      }
    };
    ...
    if (Platform.OS === 'web') {
      if (window.confirm('SeÃ§ilen sorularÄ± silmek istediÄŸinize emin misiniz?')) {
        performDelete();
      }
    } else {
      Alert.alert('KayÄ±tlarÄ± Sil', 'SeÃ§ilen sorular kalÄ±cÄ± olarak silinecektir.', [
  ...
  const handleAssignHomework = async () => {
    if (!assignTitle.trim()) {
      Alert.alert('Hata', 'LÃ¼tfen Ã¶dev baÅŸlÄ±ÄŸÄ± giriniz.');
      return;
    }
    if (selectedStudentIds.length === 0) {
      Alert.alert('Hata', 'LÃ¼tfen en az bir Ã¶ÄŸrenci seÃ§in.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('BaÅŸarÄ±lÄ±', 'Ã–dev atandÄ±.');
      setAssignOpen(false);
      setSelectedIds([]);
      setAssignTitle('');
      ...
      console.error(e);
      Alert.alert('Hata', 'Ã–dev atanÄ±rken bir sorun oluÅŸtu.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && bankQuestions.length === 0) {
    return (
      <SafeAreaView className="flex-1 justify-center items-center bg-slate-50 dark:bg-slate-950">
        ...
        <Text className="text-slate-500 font-semibold mt-3">YÃ¼kleniyor...</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      >
        <View className="flex-row items-center justify-between">
          ...
          <View className="items-center">
            <Text className="text-white text-xl font-black">Ä°Ã§erik Havuzu</Text>
            <Text className="text-indigo-100 text-xs font-medium mt-0.5">
              Soru BankasÄ± & YanlÄ±ÅŸ Havuzu
            </Text>
          </View>
        ...
        <View className="flex-row mt-4 bg-white/20 rounded-2xl p-1 gap-1">
          ...
          >
            ...
            >
              Soru BankasÄ± ({bankQuestions.length})
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => {
          ...
          >
            ...
            >
              YanlÄ±ÅŸ Havuzu ({mistakes.length})
            </Text>
          </TouchableOpacity>
        </View>
      </LinearGradient>
      ...
      <View className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 px-4 py-3 flex-row items-center justify-between gap-3">
        ...
        <View className="flex-row bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl gap-0.5">
          ...
          >
            <Text className="text-[10px] font-bold text-slate-800 dark:text-slate-200">AÃ§Ä±k UÃ§lu</Text>
          </TouchableOpacity>
        </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
                  ...
                  <View className="flex-row items-center gap-3.5">
                    ...
                    <View className="flex-1 max-w-[80%]">
                      ...
                      <Text className="text-slate-400 text-xs mt-0.5">
                        {data.count} Soru â€¢ {Object.keys(data.topics).length} Konu
                      </Text>
                    </View>
                  </View>
        ...
        {/* â”€â”€ Grid/Card and Folder Deep List View â”€â”€ */}
        {(viewLayout === 'grid' || (currentFolder.subject && currentFolder.topic)) && (
          <View className="gap-3">
            {displayedQuestions.length === 0 ? (
              <View className="items-center py-16">
                <FileQuestion size={40} color="#cbd5e1" />
                <Text className="text-slate-400 text-sm font-medium mt-2">Soru bulunamadÄ±</Text>
              </View>
            ) : (
      ...
      {/* â”€â”€ Floating Actions Bottom Bar â”€â”€ */}
      {selectedIds.length > 0 && (
        <View className="absolute bottom-5 left-4 right-4 bg-slate-900/95 dark:bg-slate-950/95 rounded-3xl p-4 flex-row items-center justify-between border border-slate-800/40 shadow-2xl">
          <View className="flex-row items-center gap-3">
            ...
            <Text className="text-white text-xs font-bold">Soru SeÃ§ildi</Text>
          </View>
          ...
          <View className="flex-row gap-2">
            ...
            >
              <Text className="text-slate-300 font-bold text-xs">VazgeÃ§</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text className="text-white font-bold text-xs">Ã–dev Ata</Text>
            </TouchableOpacity>
      ...
      {/* â”€â”€ Create / Edit Form Modal â”€â”€ */}
          ...
          <View className="bg-white dark:bg-slate-900 rounded-t-3xl p-5" style={{ maxHeight: '85%' }}>
            <View className="flex-row items-center justify-between mb-4">
              <Text className="text-lg font-black text-slate-900 dark:text-white">
                {editingQuestion ? 'Soruyu DÃ¼zenle' : 'Yeni Soru Ekle'}
              </Text>
              <TouchableOpacity
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="gap-3.5 pb-6">
              ...
              <View>
                <Text className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Soru GÃ¶rseli</Text>
                {formImageUri ? (
                  ...
                  >
                    ...
                    <Text className="text-slate-400 text-xs font-semibold mt-2">GÃ¶rsel SeÃ§</Text>
                  </TouchableOpacity>
                )}
              </View>
              ...
              {/* Title / Description */}
              <View>
                <Text className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Soru BaÅŸlÄ±ÄŸÄ± (opsiyonel)</Text>
                <TextInput
                  value={formTitle}
                  onChangeText={setFormTitle}
                  placeholder="Ã¶rn. Matematik Soru 5"
                  placeholderTextColor="#94a3b8"
              ...
              <View>
                ...
                <TextInput
                  ...
                  placeholder="Ã¶rn. ÃœslÃ¼ SayÄ±lar"
                  placeholderTextColor="#94a3b8"
                  className="bg-slate-100 dark:bg-slate-800 rounded-2xl px-4 py-3 text-sm font-semibold text-slate-900 dark:text-white"
                />
              </View>
              ...
              {formType === 'mcq' && (
                <View className="gap-3">
                  <Text className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-0.5">SeÃ§enekler</Text>
                  {['A', 'B', 'C', 'D'].map((opt) => (
                    <View key={opt} className="flex-row items-center gap-2">
                      ...
                      <TextInput
                        ...
                        placeholder={`SeÃ§enek ${opt} iÃ§eriÄŸi (opsiyonel)`}
                        placeholderTextColor="#94a3b8"
              ...
              >
                ...
                <Text className="text-white font-black text-base">
                  {editingQuestion ? 'GÃ¼ncelle' : 'Soru Ekle'}
                </Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
        ...
        <View className="flex-1 justify-end" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <View className="bg-white dark:bg-slate-900 rounded-t-3xl p-5">
            ...
            <View className="gap-3.5 pb-6">
              {/* Title */}
              <View>
                <Text className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Ã–dev BaÅŸlÄ±ÄŸÄ±</Text>
                <TextInput
                  value={assignTitle}
                  onChangeText={setAssignTitle}
                  placeholder="Ã¶rn. Matematik Hafta Sonu Ã–devi"
                  placeholderTextColor="#94a3b8"
                  className="bg-slate-100 dark:bg-slate-800 rounded-2xl px-4 py-3 text-sm font-semibold text-slate-900 dark:text-white"
                />
              </View>
              ...
              {/* Student Select */}
              <View>
                <Text className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Ã–ÄŸrenciler</Text>
                {childMembers.length === 0 ? (
                  <Text className="text-slate-400 text-xs italic">KayÄ±tlÄ± Ã§ocuk Ã¼ye bulunamadÄ±.</Text>
                ) : (
              ...
              <View className="flex-row gap-4">
                <View className="flex-1">
                  <Text className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">SÃ¼re (Dk)</Text>
                  <TextInput
                ...
                <View className="flex-1">
                  <Text className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Teslim SÃ¼resi (GÃ¼n)</Text>
                  <TextInput
                    value={assignDays}
              ...
              >
                ...
                <Text className="text-white font-black text-base">Ã–devi Ata</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
