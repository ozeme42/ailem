...
function translateType(type: string) {
  const map: Record<string, string> = {
    json: 'YazÄ±lÄ± Test', exam: 'Deneme SÄ±navÄ±', bank: 'Soru BankasÄ±',
    quick: 'HÄ±zlÄ± Test', mistake: 'YanlÄ±ÅŸ Havuzu', trackedBook: 'Kitap Takibi',
...
export default function MistakesScreen() {
  ...
  useEffect(() => {
    ...
    const unsubTests = onTestsUpdate((all) => {
      setTests(all.filter(t => t.studentId === selectedStudent.id && t.status === 'SonuÃ§landÄ±'));
      setLoading(false);
    });
  ...
  const handleShareCSV = async () => {
    ...
    try {
      ...
      Alert.alert("Hata", "CSV paylaÅŸÄ±lamadÄ±.");
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      <View className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-row justify-between items-center z-10 shadow-sm">
        <View className="flex-row items-center gap-4">
          ...
          <View>
            <Text className="text-xl font-bold bg-gradient-to-r from-red-500 to-rose-600 bg-clip-text text-transparent">YanlÄ±ÅŸlarÄ±m</Text>
        ...
        <View className="flex-1">
          ...
          <View className="bg-white dark:bg-slate-900 p-4 border-b border-slate-200 dark:border-slate-800 flex-row items-center justify-between gap-3">
            <View className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex-row max-w-xs flex-1">
              ...
              >
                <Text className={`text-[10px] font-bold ${groupingMode === 'type' ? 'text-slate-800 dark:text-white' : 'text-slate-500'}`}>SÄ±nav TÃ¼rleri</Text>
              </TouchableOpacity>
              <TouchableOpacity 
            ...
            {groupingMode === 'timeline' ? (
              ...
              <View className="space-y-4">
                ...
                <View className="bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 flex-row items-center gap-3">
                  ...
                  <TextInput
                    placeholder="SÄ±nav veya kitap ara..."
                    ...
                    <View className="flex-row items-start justify-between mb-3 flex-wrap gap-2">
                      ...
                      >
                        <Text className={`text-[8px] font-black uppercase tracking-wider ${t.isReviewed ? 'text-slate-400' : 'text-white'}`}>
                          {t.isReviewed ? "Ä°ncelendi âœ“" : "Kontrol Et"}
                        </Text>
                      </TouchableOpacity>
                    </View>
                    ...
                    <View className="space-y-2 pt-2.5 border-t border-slate-100 dark:border-slate-850">
                      {t.wrongQuestions.length > 0 && (
                        <View className="flex-row items-center flex-wrap gap-1.5">
                          <Text className="text-[8px] font-black text-slate-400 uppercase tracking-widest mr-1">YanlÄ±ÅŸlar:</Text>
                      ...
                      {t.emptyQuestions.length > 0 && (
                        <View className="flex-row items-center flex-wrap gap-1.5">
                          <Text className="text-[8px] font-black text-slate-400 uppercase tracking-widest mr-1">BoÅŸlar:</Text>
                ...
                {filteredAndSortedTimeline.length === 0 && (
                  <View className="py-20 flex-col items-center justify-center text-center">
                    ...
                    <Text className="text-slate-500 font-bold">Temiz Zaman Ã‡izelgesi! Hata Bulunmuyor.</Text>
                  </View>
                )}
              </View>
            ) : (
              ...
              <View className="flex-row flex-wrap justify-between">
                ...
                {allMistakesFlat.length === 0 && (
                  <View className="w-full py-20 items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[32px] bg-white/40 dark:bg-slate-900/10 p-6">
                    ...
                    <Text className="text-lg font-black text-slate-900 dark:text-white">MÃ¼kemmel Skor!</Text>
                    <Text className="text-slate-400 dark:text-slate-500 text-xs text-center mt-2">HenÃ¼z hiÃ§ yanlÄ±ÅŸÄ±n yok. SÄ±navlarÄ± Ã§Ã¶zmeye devam et!</Text>
                  </View>
                )}
              </View>
            )}
          ...
          <View className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-4">
            <View className="flex-row items-center justify-between mb-3 px-1">
              <Text className="text-[9px] font-black text-slate-450 uppercase tracking-widest">
                {selectedGroup} Alt DaÄŸÄ±lÄ±mÄ±
              </Text>
              {selectedSubGroup && (
                <TouchableOpacity onPress={() => setSelectedSubGroup(null)}>
                  <Text className="text-[10px] font-black text-rose-500 uppercase tracking-widest">Filtreyi KaldÄ±r</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
              ...
              >
                <Text className={`text-[10px] font-black uppercase ${!selectedSubGroup ? 'text-white' : 'text-slate-550 dark:text-slate-400'}`}>
                  TÃ¼mÃ¼ ({groups[selectedGroup].length})
                </Text>
              </TouchableOpacity>
            ...
            <View className="space-y-3">
              {tableData.map(m => (
                ...
                >
                  ...
                  <View className="flex-row items-center gap-3 shrink-0">
                    ...
                    <View className="items-center">
                      <Text className="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1.5">DoÄŸru</Text>
              ...
              {tableData.length === 0 && (
                <View className="py-20 flex-col items-center justify-center text-center">
                  ...
                  <Text className="text-slate-500 font-bold text-sm">AradÄ±ÄŸÄ±nÄ±z kriterlere uygun hata kaydÄ± bulunamadÄ±.</Text>
                </View>
              )}
            </View>
          </ScrollView>
