...
export default function NewPdfTestScreen() {
  ...
  const studentMembers = useMemo(() => 
    familyMembers.filter(m => m.role && m.role.includes('Ã‡ocuk')), 
  [familyMembers]);
  ...
  const handleSubmit = async () => {
    if (!selectedStudentId || !testSubject || !testTitle || (!fileUrl && !selectedFile)) {
      Alert.alert("Hata", "LÃ¼tfen tÃ¼m alanlarÄ± doldurun ve bir PDF ekleyin.");
      return;
    }
    ...
    if (isNaN(qCount) || qCount <= 0) {
      Alert.alert("Hata", "GeÃ§erli bir soru sayÄ±sÄ± girin.");
      return;
    }
    ...
    try {
      ...
      if (id && reassign !== 'true') {
        ...
        Alert.alert("BaÅŸarÄ±lÄ±", "Test baÅŸarÄ±yla gÃ¼ncellendi.", [
        ...
        Alert.alert("BaÅŸarÄ±lÄ±", "Yeni test baÅŸarÄ±yla oluÅŸturuldu.", [
          { text: "Tamam", onPress: () => router.back() }
        ]);
      }
    } catch (e) {
    ...
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      <View className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-row items-center z-10 shadow-sm">
        ...
        <View>
          <Text className="text-lg font-extrabold text-slate-900 dark:text-white leading-none">
            {isEditing ? 'Testi DÃ¼zenle' : (reassign === 'true' ? 'Testi Tekrar Ata' : 'Yeni PDF Testi')}
          </Text>
          <Text className="text-[9px] font-black text-rose-500 uppercase tracking-widest mt-1">PDF FormatÄ±nda SÄ±nav</Text>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        <View className="space-y-6">
          ...
          <View>
            <Text className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider mb-2">SÄ±nav BaÅŸlÄ±ÄŸÄ±</Text>
            <TextInput
              placeholder="Test AdÄ±..."
          ...
          <View>
            <Text className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider mb-2">Atanacak Ã–ÄŸrenci</Text>
          ...
          <View>
            <Text className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider mb-2">Ders SeÃ§imi</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
              {Object.keys(curriculumMap).length > 0 ? Object.keys(curriculumMap).map((subj) => {
                ...
                <Text className="text-slate-500 text-xs italic">MÃ¼fredat yÃ¼kleniyor...</Text>
              )}
            </ScrollView>
          </View>
          ...
          {Boolean(testSubject) && curriculumMap[testSubject] && curriculumMap[testSubject].length > 0 && (
            <View>
              <Text className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider mb-2">Konu SeÃ§imi</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
          ...
          <View className="flex-row gap-2">
            ...
            <View className="w-[85px]">
              <Text className="text-[10px] font-bold text-slate-450 dark:text-slate-400 uppercase tracking-wider mb-2">Ä°lk Soru</Text>
              <TextInput
            ...
            {selectedFile ? (
              ...
              <View className="space-y-4">
                ...
                >
                  ...
                  <Text className="text-sm font-bold text-slate-700 dark:text-slate-300">Cihazdan PDF SeÃ§</Text>
                  <Text className="text-[10px] text-slate-400 mt-1">veya tarayÄ±cÄ±dan indirdiÄŸiniz dosyayÄ± seÃ§in</Text>
                </TouchableOpacity>
          ...
          >
            ...
            <Text className="text-sm font-bold text-slate-700 dark:text-slate-300">
              {Object.keys(answerKey).length > 0 ? `${Object.keys(answerKey).length} Cevap Girildi (DÃ¼zenle)` : 'Optik Cevap AnahtarÄ± OluÅŸtur'}
            </Text>
          </TouchableOpacity>
            ...
            {isSubmitting ? (
              ...
              <>
                ...
                <Text className="text-white font-black text-sm uppercase tracking-wider">
                  {isEditing ? 'DeÄŸiÅŸiklikleri Kaydet' : (reassign === 'true' ? 'Tekrar Ata' : 'Ã–devi GÃ¶nder')}
                </Text>
              </>
            )}
          </TouchableOpacity>
        ...
        <View className="flex-1 bg-black/60 pt-20">
          <View className="flex-1 bg-white dark:bg-slate-900 rounded-t-3xl shadow-2xl border-t border-slate-200 dark:border-slate-800 overflow-hidden">
            <View className="p-4 border-b border-slate-200 dark:border-slate-800 flex-row justify-between items-center bg-slate-50 dark:bg-slate-950">
              <View>
                <Text className="text-lg font-bold text-slate-900 dark:text-white">Cevap AnahtarÄ±</Text>
                <Text className="text-xs text-slate-500">{questionCount} Soru Ä°Ã§in</Text>
              </View>
            ...
            <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
              ...
              <View className="mb-6 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200 dark:border-slate-800">
                <Text className="text-xs font-bold text-slate-500 mb-2">HÄ±zlÄ± Toplu GiriÅŸ (YapÄ±ÅŸtÄ±r)</Text>
                <TextInput
                  placeholder="Ã–rn: ABADCBE..."
                  placeholderTextColor="#94a3b8"
                  value={bulkText}
