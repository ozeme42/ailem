...
export function safeParseDate(dateStr: any): Date {
  ...
  // Try custom Turkish format e.g. "15 AÄŸustos 2024" or "01 AÄŸustos 2024"
  try {
...
export const addBookToMemberLibrary = async (fid: string, mid: string, bid: string) => {
    // 1. Ãœyeye ait var olan kÃ¼tÃ¼phane belgesini bul (ID'si mid olmayabilir!)
    ...
    // Kitap daha Ã¶nce eklenmiÅŸ mi kontrol et
    ...
    // mediaItems koleksiyonundaki readers dizisini de gÃ¼ncelle
...
export const removeBookFromMemberLibrary = async (fid: string, mid: string, bid: string) => {
    ...
    // mediaItems koleksiyonundaki readers dizisinden Ã§Ä±kar
    const bookRef = doc(db, 'mediaItems', bid);
    const bookSnap = await getDoc(bookRef);
    if (bookSnap.exists()) {
...
export const updateUserBookStatus = async (fid: string, mid: string, bid: string, status: string, progress?: number, summaryImages?: string | string[]) => {
    ...
    for (const libDoc of querySnapshot.docs) {
        ...
        if (idx !== -1) {
            ...
            // Okunuyor durumuna geÃ§tiyse ve baÅŸlama tarihi yoksa ekle
            if (status === 'reading' && !books[idx].startedAt) {
                books[idx].startedAt = new Date().toISOString();
            }
            
            // Bitti durumuna geÃ§tiyse bitiÅŸ tarihini ve %100 ilerlemeyi kaydet
            if (status === 'finished') {
            ...
            return; // Bulduk ve gÃ¼ncelledik, diÄŸer fragmanlara bakmaya gerek yok
        }
    }
};
...
export const linkStudentToTeacher = async (studentId: string, teacherCode: string): Promise<boolean> => {
    try {
        const q = query(collection(db, 'users'), where('teacherCode', '==', teacherCode.toUpperCase()));
        const snap = await getDocs(q);
        if (snap.empty) {
            console.error('Teacher code not found');
            return false;
        }
        ...
        console.error('Error linking student to teacher', e);
        return false;
    }
}
