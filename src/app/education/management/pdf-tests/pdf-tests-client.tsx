...
import { NewPdfTestForm } from "@/components/new-pdf-test-form";
...
export function PdfTestsClient() {
  const { familyId, familyMembers } = useAuth();
  const { toast } = useToast();
  const [pdfTests, setpdfTests] = React.useState<Test[]>([]);
  ...
  React.useEffect(() => {
    ...
    const unsub = onTestsUpdate((allTests) => {
      setpdfTests(allTests.filter(t => t.sourceType === 'pdf'));
      setLoading(false);
    });
  ...
  const handleFormSubmit = async (data: Omit<Test, 'id' | 'familyId'>) => {
    try {
      if (editingTest && !reassigningTest) {
        ...
        toast({ title: "✅ pdf Test Güncellendi" });
      } else {
        await addTest(data);
        toast({ title: reassigningTest ? "✅ Test Yeni Görev Olarak Atandı" : "✅ pdf Testi Oluşturuldu" });
      }
      setIsFormOpen(false);
      setEditingTest(null);
        ...
        <header className={themeColors.HEADER_BG}>
            <div className="max-w-6xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl sm:text-2xl font-black tracking-tight text-slate-100 leading-none">pdf Testler</h1>
                ...
                <Button onClick={() => { setReassigningTest(null); handleOpenForm(null); }} className={cn("rounded-xl h-11 px-5 font-bold", themeColors.BUTTON_PRIMARY)}>
                    <PlusCircle className="mr-2 h-5 w-5" /> <span className="hidden sm:inline">Yeni pdf Test</span>
                </Button>
            </div>
        </header>
                ...
                <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 flex flex-col gap-4">
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                        <TabsList className={themeColors.TAB_LIST}>
                            <TabsTrigger value="active" className={themeColors.TAB_TRIGGER}>Aktif pdf Testler ({pdfTests.length})</TabsTrigger>
                        </TabsList>
                    </div>
                </div>
                ...
                <TabsContent value="active" className="m-0 animate-in fade-in zoom-in-95 duration-300">
                    {pdfTests.length > 0 ? (
                                                            ...
                                                            <AlertDialogContent className="bg-slate-900 border-slate-800 text-slate-100 rounded-2xl">
                                                                <AlertDialogHeader>
                                                                    ...
                                                                    <AlertDialogDescription className="text-slate-400">
                                                                        "{test.title}" pdf testi kalıcı olarak silinecektir.
                        ...
                        <div className="flex flex-col items-center justify-center py-24 text-center bg-slate-900/30 rounded-3xl border border-dashed border-slate-800 m-auto max-w-2xl w-full">
                            ...
                            <h3 className="text-xl font-bold text-slate-200">pdf Test Bulunamadı</h3>
                            <p className="text-slate-400 mt-2 text-sm max-w-xs mx-auto">Yeni bir pdf tabanlı test oluşturarak başlayın.</p>
            ...
            <DialogContent className="max-w-4xl h-[90vh] flex flex-col bg-slate-900 border-slate-800 text-slate-100 rounded-3xl p-0 overflow-hidden shadow-2xl">
                <DialogHeader className="p-6 pb-2 border-b border-white/5 bg-white/5">
                    <DialogTitle className="text-xl font-bold flex items-center gap-2">
                        ...
                        {reassigningTest ? 'pdf Testini Tekrar Ata' : editingTest ? 'pdf Testini Düzenle' : 'Yeni pdf Testi Oluştur'}
                    </DialogTitle>
                </DialogHeader>
