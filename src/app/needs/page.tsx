...
const brightColors = [
    { id: 'yellow-amber', name: 'Sarı', gradient: 'from-yellow-300 to-amber-400' },
    { id: 'rose-pink', name: 'Somon', gradient: 'from-rose-400 to-pink-500' },
    { id: 'emerald-teal', name: 'Yeşil', gradient: 'from-emerald-400 to-teal-500' },
    { id: 'sky-blue', name: 'Mavi', gradient: 'from-sky-400 to-blue-500' },
    { id: 'violet-purple', name: 'Menekşe', gradient: 'from-violet-400 to-purple-500' },
    { id: 'orange-red', name: 'Turuncu', gradient: 'from-orange-400 to-red-500' },
];
...
const ListCard = ({ list, colorClass, onClick, onEdit, onDelete }: { 
    ...
    return (
        <div className="relative group rounded-lg overflow-hidden shadow-sm" onClick={onClick}>
            <div className={cn("flex items-center text-white p-4 cursor-pointer h-20", colorClass)}>
                 <div className="flex-grow">
                    <p className="text-xl font-bold leading-tight truncate">{list.name}</p>
                    <p className="text-white/90 text-sm font-normal truncate">
                        {description}
                    </p>
                </div>
                <ChevronRight className="h-6 w-6 text-white/70 ml-auto shrink-0"/>
            </div>
             <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-white hover:text-white hover:bg-black/20" onClick={(e) => e.stopPropagation()}>
                ...
                shoppingLists.map((list, index) => {
                    ...
                    return (
                        <ListCard 
                            key={list.id} 
                            list={list} 
                            colorClass={cn("bg-gradient-to-r", color.gradient)} 
